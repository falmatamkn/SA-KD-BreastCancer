...
def load_datasets(train_dir, test_dir, seed=42):
    """
    Load the pre-split BreakHis 400x dataset.

    The provided training set is divided into:
        90% training
        10% validation

    The test set remains completely separate and is used only
...
def preprocess_images(images, labels, training=False):
    """
    Prepare images for the teacher and student models.

    Images remain in their standard pixel range here.

    Student 1/255 rescaling is performed inside the student model.
    EfficientNetB0 handles its required input preprocessing internally.
...
def prepare_datasets(train_ds, val_ds, test_ds):
    """
    Apply augmentation/preprocessing and prefetch the datasets.
    """

    train_ds = train_ds.map(
        lambda x, y: preprocess_images(x, y, training=True),
        num_parallel_calls=AUTOTUNE,
    )

    val_ds = val_ds.map(
        lambda x, y: preprocess_images(x, y, training=False),
        num_parallel_calls=AUTOTUNE,
    )
